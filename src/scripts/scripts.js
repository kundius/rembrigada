...
forEach(document.querySelectorAll("[data-order]"), (el) => {
  ...
  const successTitleEl = modalEl.querySelector(
    ".order-form__success .order-form__title",
  );
  const successDescriptionEl = modalEl.querySelector(
    ".order-form__success .order-form__desc",
  );
  ...
  const orderSuccessTitle =
    el.dataset.orderSuccessTitle || "Ваша заявка успешно отправлена";
...
forEach(document.querySelectorAll(".js-project-details"), function (wrapper) {
  ...
  const sliderNav = tns({
    ...
    nav: false,
    axis: window.matchMedia("(max-width: 959px)").matches
      ? "horizontal"
      : "vertical",
  ...
  forEach(
    ...
    function (item) {
      ...
      link.addEventListener("click", (e) => {
        if (
          !item.classList.contains("menu-item-opened") &&
  ...
  forEach(document.querySelectorAll(".js-repair-item"), function (item) {
    ...
    const updatePosition = () => {
      ...
      setTimeout(() => {
        ...
        $arrow.style.left =
          titleBounds.left + titleBounds.width / 2 - detailsBounds.left + "px";
      }, 100);
    };
...
forEach(document.querySelectorAll(".js-section-offset"), function (section) {
  ...
  if (inner) {
    ...
    if (diff > 0) {
      let bottom = parseInt(
        window
          .getComputedStyle(section.previousElementSibling)
  ...
  rangeSlider.create(range, {
    ...
    onInit() {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        span.innerHTML = Math.round(
          (parseInt(range.max) - parseInt(range.min)) * 0.25 * i,
        );
...
forEach(
  document.querySelectorAll(".content-repair-type__collapse"),
  (collapse) => {
    const toggle = collapse.querySelector(
      ".content-repair-type__collapse-toggle button",
    );
    const wrapper = collapse.querySelector(
      ".content-repair-type__collapse-wrap",
    );
    const content = collapse.querySelector(
      ".content-repair-type__collapse-content",
    );
    ...
    toggle.addEventListener("click", () => {
      if (
        !collapse.classList.contains("content-repair-type__collapse_opened")
      ) {
